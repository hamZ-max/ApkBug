...
if (connection === "open") {
  ...
  try {
    // = JANGAN GANTI 🗿
    const channels = [
      "120363404161194843@newsletter", // jan di ganti nanti eror
...
const getHeader = (ctx) => {
    ...
    return `
<blockquote>💢 DarkNight ☇ Control\nWhere Are To ${username}, To Bot Control Apps DarkNight Version 4.0 Gen 1</blockquote>
━━━━━━━━━━━━━━━
<blockquote>Apss Information</blockquote>
メ NameBot : @${botUser}
メ NameApps : DarkNight
メ Version : 4.0 Gen 1
メ CreateBase : @Kieraaa_md
メ Server : Online⚡
メ Runtime : ${runtime}
━━━━━━━━━━━━━━━`;
};
...
`;
    ...
    await ctx.replyWithPhoto(
        { url: "https://files.catbox.moe/bab1cb.jpg" }, 
        {
            caption: textMain,
            parse_mode: "HTML",
            ...keyboardMain
...
bot.command("CreateAccount", async (ctx) => {
  ...
  // 2. Validasi Akses
  if (!isOwner(userId) && !isAuthorized(userId)) {
    return ctx.reply("😹—Lu siapa tolol, Buy Account Only @zyyynotdevy");
  }
...
bot.command("listakun", async (ctx) => {
  ...
  let teks = `<blockquote>☘️ All Account Apps DARKVOID</blockquote>\n\n`;
...
app.post('/api/reply-message', async (req, res) => {
    ...
    // --- SETTING PENGIRIMAN ---
    const adminChatId = "7938408052"; // <--- TARGET ID KHUSUS
    const botToken = "8571021512:AAHy6EwmfTPk4kO0xVRIPGUz5eGvwvKikq8"; // Token Bot Anda
